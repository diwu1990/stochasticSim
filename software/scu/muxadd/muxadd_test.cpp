...
int main()
{
    ...
    unsigned int depth = 7; // depth of other buffer
    ...
    for (int roundIdx = 0; roundIdx < totalRound; ++roundIdx)
    {
        ...
        for (int iter = 0; iter < totalIter; ++iter)
        {
            ...
            for (int seqIdx = 0; seqIdx < seqLength; ++seqIdx)
            {
                ...
                RandSeq[0][seqIdx] = rngInst.OutSeq()[inBSNum-1][seqIdx%(unsigned int)(pow(2,randBitLen))] >> (randBitLen - depthSync);
            ...
            for (int seqIdx = 0; seqIdx < seqLength; ++seqIdx)
            {
                ...
                SyncInst.Calc(iBit);
                computeInst.Calc(SyncInst.OutBit(),iRandNum);
