...
void SOBOL::Init(unsigned int param1, unsigned int param2, unsigned int param3, string param4)
{
    ...
    {
        printf("Error: Input dimension index exceeds the legal range (1~1111) in SOBOL class.\n");
    }
    if (sobolLen < 4 || sobolLen > 16)
    {
        printf("Error: Input sobolLen exceeds the legal range (4~16) in SOBOL class.\n");
    }
    ...
    dirVec.resize(sobolLen);
    dirMem.resize(sobolLen);
...
void SOBOL::Report()
{
    printf("Current SOBOL:\n");
    std::cout << "Instance name:          " << m_name << std::endl;
    printf("SOBOL Length:           %-5u\n", sobolLen);
    printf("Dimension Index:        %-5u\n", dimNum);
...
void SOBOL::Help()
{
    printf("**********************************************************\n");
    printf("**********************************************************\n");
    printf("Calling SOBOL Help. Following are instructions to SOBOL Instance Usage:\n");
    printf("1. inst.Init() method:\n");
    printf("Configure the SOBOL inst.\n");
    printf("Initial Parameters: SOBOL Bit Length, Dimension Index, Instance Name.\n");
    ...
    printf("Get the configuration of the current SOBOL instance.\n");

    printf("5. inst.SeqPrint() method:\n");
    printf("Print the content in the output sequence of current SOBOL instance.\n");

    printf("6. inst.VecPrint() method:\n");
    printf("Print the content in the directional vector of current SOBOL instance.\n");

    printf("7. inst.MemPrint() method:\n");
    printf("Print the content in the directional memory value of current SOBOL instance.\n");

    printf("8. inst.SeqLen() method:\n");
    printf("Return the length of the sequence of current SOBOL instance.\n");
    printf("**********************************************************\n");
    printf("**********************************************************\n");
}
...
void SOBOL::SeqPrint()
{
    printf("Calling SeqPrint for SOBOL instance: ");
...
void SOBOL::VecPrint()
{
    printf("Calling VecPrint for SOBOL instance: ");
    std::cout << m_name << std::endl;
    for (int i = 0; i < sobolLen; ++i)
    {
        printf("%u,", dirVec[i]);
...
void SOBOL::MemPrint()
{
    printf("Calling MemPrint for SOBOL instance: ");
    std::cout << m_name << std::endl;
    for (int i = 0; i < sobolLen; ++i)
    {
        printf("%u,", dirMem[i]);
...
//  when dimension is 1, all dimensional vectors are initialized to 1.
    for ( j = 0; j < maxcol; j++ )
    {
      v[0][j] = 1;
    }
...
//
      j = poly[i]; // proper polynomial for each dimension
      m = 0;
      ...
      // m is the number of degree of the polynomial.
      // printf("%d\n", m);
//
...
//
      ...
      {
        j2 = j / 2;
        includ[k] = ( j != ( 2 * j2 ) ); // set includ[k] true if j2 is not exactly half of j, which means LSB is 1
        j = j2;
      }
      // includ indicates the position with a 1 in the polynomial, not including the last constant 1 bit in the polynomial.
//
//  Calculate the remaining elements of row I as explained
//  in Bratley and Fox, section 2.
//
      // printf("%d\n", maxcol);
      // maxcol is always 30 under current 1111 dim setting.
      // following code generate all directional vectors
      for ( j = m; j < maxcol; j++ )
      {
        newv = v[i][j-m];
        // printf("%d\n", newv);
    ...
    {
        directionalVec[j] = v[dim_num-1][j];
        // printf("%d\n", directionalVec[j]);
    }
//
...
//
  ...
  if ( *seed == 0 )
  {
    // initialize first output random number for each dim with 0 
    l = 1;
    for ( i = 0; i < dim_num; i++ )
    {
      lastq[i] = 0;
  ...
  {
    // if new seed (index of random number requested) is the next of seed_save (index of last random number requested)
    // get the rightmost zero position
    l = i4_bit_lo0 ( *seed );
  }
  else if ( *seed <= seed_save )
  {
    // if new seed (index of random number requested) is larger than the next of seed_save (index of last random number requested)
    // restart the random number generation, and find the correct lastq value
    seed_save = 0;
    l = 1;
    for ( i = 0; i < dim_num; i++ )
    {
